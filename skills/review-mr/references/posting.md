...
Both repos' Duo instructions and code review guidelines use
...
- **Shape of every comment** (the reader is assumed to know nothing about this corner):
  ...
  2. **Context, quoted and linked.** Whatever the comment leans on is copied verbatim as a
     ...
     `The [issue](https://gitlab.com/…/605985) description says:` followed by
     `> Reconciles with the existing aggregate UsersUsage.dailyUsage when only one user has usage.`
     For code: `[`app/models/x.rb:20`](https://gitlab.com/…/blob/<sha>/app/models/x.rb#L20)`
...
- **Local reproductions are scenarios, not dumps.** Never paste local subscription names,
  customer/user ids, wallet ids, or seed dates into a comment; nobody else can resolve
  them. Say what the setup was in plain words and what the two sides showed, keeping only
  the numbers whose relationship carries the point (e.g. "the consumer-wallet series
  totalled 58 for the day, the event-based series 75, because two of the three users had
  exhausted their 24-credit allocation"). Ids and scripts live in the record's artifacts
...
- **The label is bold.** The Conventional Comments line is written `**issue (non-blocking):** subject` so it stands out in the GitLab UI; same in the summary's thread list.
- **Blocking is explicit.** Mark `(blocking)` or `(non-blocking)` on every `issue` and
  `suggestion`. The GitLab guideline: unmarked feedback delays the MR.
- **Assume the author considered alternatives.** Phrase as "What do you think about X
  here, given Y?" not "You should X."
- **Suggestion blocks** (```` ```suggestion ````) only when the replacement is exact and
  compiles; otherwise describe.
- Inclusive language; "CustomersDot" not "CDOT".
- Never mention Claude, AI, or that the review was assisted. The user is the reviewer.

## Where a comment lands

A finished review has exactly two kinds of outbound comment, mirroring a review done in
the GitLab UI:
...
2. **One summary note** on the MR page, posted last, as a **reply to the author's review
   request thread** when one exists (find it in the discussions: the author's note that
   @-mentions the user asking for review; reply with `$P/discussions/<id>/notes`). It opens
...
| D1 | award | note 3796401338 | thumbsup | draft |
| D2 | reply | discussion 0c53... | ... | draft |
| F1 | diffnote | app/x.rb:112 | ... | draft |
| E1 | note | @expert | ... | draft |
...
**Pre-flight, immediately before the first post (added 2026-09-11 after !17041):** fetch the MR and
assert `state == "opened"`, `sha` equals the head the drafts were anchored to, and re-read the
discussions for any review posted since Phase 0 (another reviewer, a maintainer, an approval). If the
MR is merged or closed, **stop and tell the user**; blocking labels on a merged MR are meaningless and
the comments may belong on a follow-up MR instead. If another reviewer's thread covers a queued point,
that item goes back to `draft` and is re-shown with the earlier thread quoted and linked. On !17041 a
reviewer approved, a maintainer merged, and a same-line duplicate was posted because only `sha` was
re-checked, not `state` or the discussions.
...
P="projects/gitlab-org%2F<project>/merge_requests/<iid>"

# glab gotchas (verified 2026-09-07 on !17037):
...
# reply in an existing discussion (Duo thread, roulette thread, ...)
$GL api "$P/discussions/<discussion_id>/notes" -f "body=$(cat /path/to/text.md)"

# 👍 on a note (Duo agreement)
...
# labels (only if the user asked; e.g. after DB review as maintainer)
$GL mr update <iid> --label "database::approved" --unlabel "database::review pending"
```
...
2. Immediately before the PUT, fetch again and compare `updated_at` to the recorded value.
   **If it moved, do not write.** Print the current server body to the user, say when it
   ...
   hand-edited the note between the two reads; a PUT would erase that edit and GitLab keeps
   no note history to recover it from (this happened on !17037, note 3798147157, 14:49Z).
...
```sh
set -e
GL=/opt/homebrew/opt/glab/bin/glab; P="projects/gitlab-org%2F<project>/merge_requests/<iid>"
SEEN="2026-09-07T14:12:20.363Z"   # updated_at recorded when the draft was made
...
Never call `glab mr approve`, `glab mr merge`, or resolve a discussion.

**Head guard (verified 2026-09-11 on !17049).** Re-read `diff_refs` immediately before every POST and stop if `head_sha` differs from the SHA the review was done against. When it fires, classify the move before doing anything else: `git fetch` the branch, `git log --oneline <old>..FETCH_HEAD ^origin/main` (new commits?) and `git diff --stat <old> FETCH_HEAD -- <the MR's files>` (content change?). A pure rebase (same commits, MR files unchanged, anchor line still reads the same) → reset the worktree to the new head, repoint blob links in the body to the new SHA, update the guard, post. Any content change → back to Phase 4 for the changed hunks; drafts that quote moved lines are stale.
